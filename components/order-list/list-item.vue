<template>
	<view class="list-warp">
		<view v-if="orderList && orderList.length > 0">
			<view class="list" hover-class="order-list-hover-style" v-for="(item,index) in orderList" :key="index" @tap="onOrderItemClick(item)">
				<view class="list-top">
			...
			<view class="example-body">
				<uni-load-more :status="status" :content-text="contentText" showIcon @clickLoadMore="clickLoadMore" />
			</view>
		</view>
...
<style lang="scss" scoped>
	.list-warp {
		...
		.list {
			...
			// box-shadow: 0 -5rpx 0 0 #CCCCCC inset;
